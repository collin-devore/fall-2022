...
#=
It seems reasonable right now to conclude that the estimates from b and c are globally concave. The coeffecients are pretty consistent between the two, lending evidence to the idea that there is only a single equilibrium. Hence, it is globally concave.
=#
...
#=
I am having quite a bit of trouble getting this to work. It seems that the error I am having is coming from the specification of D. If I do not set it, I get an error saying that D is not defined. If it is in the function, an array issue is created at [1,2] on line 148. The same is true if I set it above the value of 1.
=#
